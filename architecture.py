...
def load_transform_label_train_data(directory, representation):
    ...
    for category in CATEGORIES:
        ...
        for f in files:
            ...
            img = raw_image_to_representation(img, representation)
            dictionnary[f] = (img, category[1])
...
def learn_model_from_data(train_data, algo_dico):
    """result = list(zip(*train_data))
    X = np.asarray(result[0])
    y = np.asarray(result[1])
    print(X.shape)"""
    X = train_data["X"]
    y = train_data["y"]
    algo_dico["model"].set_params(**algo_dico["param"])
    algo_dico["model"].fit(X, y)
    
    return algo_dico["model"]
...
def estimate_model_score(train_data, model, k):
    return cross_val_score(model, train_data["X"], train_data["y"], cv=k)
